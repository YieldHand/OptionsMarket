...
contract Core is ReentrancyGuard {
    ...
    mapping(address => mapping(address => mapping(bool => mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256))))))
        public orderbook;
    mapping(address => mapping(address => mapping(bool => mapping(uint256 => mapping(uint256 => uint256)))))
        public positions;
    ...
    event OptionExcersize(
        uint256 optionId,
        uint256 excersizeCost,
        uint256 timestamp
    );
    event Transfer(
    ...
    function excersizeOption(uint256 purchaseId) public returns (bool) {
        require(
            optionPurchases[purchaseId].exercized == false,
            "This option has already been excersized"
        );
        require(
    ...
    function excersizeOptions(uint256[] memory purchaseIds)
        public
        returns (bool)
    {
    ...
    function sellOption(
        ...
        require(
            ...
            "Could not transfer the amount from msg.sender that was requested"
        );
        if (
    ...
    function buyOptionByExactPremiumAndExpiry(
        ...
        require(
            optionIsBuyable,
            "Sorry: there is no one selling options that meet your specifications. Perhaps try buyOptionByIds"
        );
    ...
    function cancelOptionOffer(uint256 offerId) public returns (bool) {
        ...
        require(
            ...
            "The msg.sender has to be the seller"
        );
    ...
    function transfer(
        ...
        //Transfer the amount of options from the msg.sender to the recipient address
    ...
    function approve(
        ...
        //allows the designee to spend an amount of options
        require(
        ...
        require(
            ...
            "Cannot approve more than owned"
        );
    ...
    function approval(
        ...
        address designee,
        uint256 purchaseId
    ) public returns (uint256 approvalAmount) {
        //return the amount of options the designee can spend
    ...
    function _transfer(
        ...
        require(
            optionPurchases[purchaseId].amountUnderlyingToken >= amount,
            "Cannot tranfer more than owned"
        );
        ...
        ][optData.strikePrice][optData.expiry].sub(amount); //adjust the position of the owner
        positions[recipient][optData.token][optData.isCallOption][
            optData.strikePrice
        ][optData.expiry] = positions[recipient][optData.token][
            optData.isCallOption
        ][optData.strikePrice][optData.expiry].add(amount); //adjust the position of the reciever
